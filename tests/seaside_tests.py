...
class TestSeaside(tornado.testing.AsyncTestCase):
	...
	def test_Lookout(self):
		tu.print_test_header("test Lookout")
		lookout = sea.Lookout(self.game, self.player1)

		for i in range(5):
			self.player1.topdeck()

		self.player1.gain_to_deck('Cellar')
		self.player1.gain_to_deck('Chapel')
		self.player1.gain_to_deck('Feast')
		self.player1.gain_to_deck('Chancellor')

		lookout.play()

		yield tu.send_input(self.player1, "post_selection", ["Chancellor"])

		self.assertTrue(len(self.game.trash_pile) == 1)

		yield tu.send_input(self.player1, "post_selection", ["Feast"])
		self.assertTrue(len(self.player1.discard_pile) == 1)
		self.assertTrue(len(self.player1.hand) == 6)

		lookout.play()

		yield tu.send_input(self.player1, "post_selection", ["Cellar"])
