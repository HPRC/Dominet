...
class Lookout(crd.Card):
	def __init__(self, game, played_by):
		...
		self.title = "Lookout"
		self.price = 3
		self.description = "{} Look at the top 3 cards of your deck. Trash one of them. " \
		                   "Discard one of them. Put the other one on the top of your deck.".format(crd.format_actions(1))
		self.type = "Action"
	...
	def play(self, skip=False):
		...
		for i in range(3):
			...
			if topdeck is not None:
				topdeck_list.append(topdeck)

		empty = False
		if len(topdeck_list) > 0:
			to_trash = yield self.played_by.select(1, 1, crd.card_list_to_titles(topdeck_list), 'Choose one to trash')
			to_trash = self.remove_selection(topdeck_list, to_trash)

			self.played_by.hand.add(to_trash)
			yield self.played_by.discard([to_trash.title], self.game.trash_pile)
			self.game.announce('-- trashing {}'.format(to_trash.log_string()))

			self.game.update_trash_pile()

			if len(topdeck_list) > 0:
				to_discard = yield self.played_by.select(1, 1, crd.card_list_to_titles(topdeck_list), 'Choose one to discard')
				to_discard = self.remove_selection(topdeck_list, to_discard)

				self.played_by.hand.add(to_discard)
				yield self.played_by.discard([to_discard.title], self.played_by.discard_pile)
				self.game.announce('-- discarding {}'.format(to_discard.log_string()))

				if len(topdeck_list) > 0:
					to_hand = topdeck_list.pop()
					self.played_by.hand.add(to_hand)
					self.played_by.update_hand()
					self.game.announce('-- drawing {}'.format(to_hand.log_string()))
				else:
					empty = True
			else:
				empty = True
		...
		if empty is True:
			self.game.announce("-- until their deck was empty")

		crd.Card.on_finished(self)

	def remove_selection(self, topdeck_list, selection):
		count = 0
		for td in topdeck_list:
			if td.title == selection[0]:
				return topdeck_list.pop(count)
			else:
				count += 1
